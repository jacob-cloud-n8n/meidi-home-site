...
- `圖片大小`：小 / 標準 / 大 / 特大；前台預設 `大`，`特大` 會接近卡片寬度的 8 成。
- `字體大小`：控制卡片標題與說明文字。
- `排序`
- `啟用`

目前前台最多顯示六張服務卡。建議排序如下：
...
精選案例已改為「案例專案 + 案例空間」兩層式維護。`/portfolio/` 只顯示案例卡片；單一案例內容顯示在 `/portfolio/案例代號/`。舊的 `類型=案例` 仍可顯示，但只作為過渡資料；正式案例建議改用以下新格式。

#### 案例專案

一個完整服務案建立一列，負責列表卡片、案例總覽、亂源分析與解決方案。

- `名稱`：建議使用穩定 key，例如 `case-project.liu-home-storage`。
- `前台位置`：例如 `精選案例 / 全屋整理 / 全屋收納案例`，前台可取最後一段作為標題。
- `類型`：填 `案例專案`。
- `Slug` 或 `網址代碼`：案例代號，例如 `liu-home-storage`；後續案例空間用此代號綁定。
- `分類`：例如 `全屋整理`、`玄關`、`更衣間`。
- `摘要` 或 `文字內容`：案例列表與詳情頁摘要。
- `封面圖` 或 `封面圖網址`：案例卡片主圖，需使用壓縮與浮水印後圖片。
- `亂源分析`：可用換行或分號分隔，前台會顯示為條列。
- `解決方案` 或 `美地做法`：可用換行或分號分隔，前台會顯示為條列。
- `授權狀態`：例如 `待授權`、`案例預覽`、`已授權`。
- `字體大小`
- `排序`
- `啟用`
...
同一個服務案的每個空間建立一列，負責詳情頁的空間成果區塊。

- `名稱`：建議使用穩定 key，例如 `case-space.liu-home-storage.living-a`。
- `前台位置`：例如 `精選案例 / 全屋整理 / 全屋收納案例 / 客廳 A 視角`。
- `類型`：填 `案例空間`。
- `所屬案例` 或 `案例Slug`：填對應案例專案的 `Slug`，例如 `liu-home-storage`。
- `空間名稱`：例如 `客廳 A 視角`、`衣帽間 A 區`。
- `文字內容`：此空間的整理方法或成果摘要。
- `案例圖片` 或 `案例圖片網址`：建議使用 PDF 轉出的整頁案例圖，需壓縮與浮水印。
- `整理前圖片` / `整理後圖片`：舊 Before / After 欄位仍相容，但新案例可優先使用 `案例圖片`。
- `排序`
- `啟用`

目前網站內建一筆 `liu-home-storage` PDF 案例草稿作為版型示範；若 Notion 建立同 slug 的 `案例專案`，前台會以 Notion 資料覆蓋內建草稿。
...
目前前台至少保留十個案例分類。Notion `分類` 欄位可保留舊選項供既有案例過渡使用，並新增以下建議選項：
...
- `team.stories.hero.image`：收納大小事 / 頁首主視覺圖片
- `team.stories.hero.badge`：收納大小事 / 頁首圖片中央標籤
- `team.stories.label`：收納大小事 / 文章區標籤
- `team.stories.title`：收納大小事 / 主標題
- `team.stories.lede`：收納大小事 / 說明文字
...
文章列的 `名稱` 會成為前台文章標題；`Slug` 會成為子頁網址，例如 `/team/three-stage-routine/`。若未來使用 `article.1` 這類管理代號作為名稱，前台會改抓 `前台位置` 最後一段作為顯示標題。`字體大小` 會套用於文章卡片標題與摘要，以及文章詳情頁標題與內文。若未設定，前台使用較保守的標準字級。
