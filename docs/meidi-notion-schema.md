...
| 收納團隊 | `team.profile.image` | 收納團隊 / 華琍老師形象照 |
| 收納團隊 | `team.certificate.image` | 收納團隊 / 服務證書 |
| 收納團隊 | `team.credential.1.image` | 收納團隊 / 空間診斷圖示 |
| 收納團隊 | `team.credential.2.image` | 收納團隊 / 親子生活圖示 |
| 精選案例 | 案例列 | 精選案例 / 每張案例卡可設定 `圖片` |
...
- 收納團隊：Hero、華琍老師介紹、資歷/社群/動態牆、證書與兩張資歷卡。
...
- `名稱`：案例 key 或案例名稱。
- `前台位置`：例如 `精選案例 / 衣櫥收納 / 換季衣櫥整理`，前台會取最後一段作為案例標題。
- `類型`：填 `案例`。
- `分類`：建議 select 或文字；篩選選單會依此欄位自動產生。
- `文字內容`：案例摘要。
- `圖片` 或 `圖片網址`：案例照片；正式案例需授權與去識別化。
- `授權狀態`：例如 `待授權`、`已授權`。
- `排序`
- `啟用`
